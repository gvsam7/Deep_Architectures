...
Description: Test platform to validate various Deep architectures using SSRP dataset. The dataset is comprised of aerial
            ...
            Tested images: compressed 50x50, 100x100, and 226x 226 pixel images. Note that 50x50 was too small for the
            5 CNNs.
...
def main():
    ...
    X_trainval, X_test, y_trainval, y_test = train_test_split(X1, y1, test_size=0.2, stratify=y1, random_state=21, shuffle=True)
    test = X_test
    X2 = X_trainval
    y2 = y_trainval
    X_train, X_val, y_train, y_val = train_test_split(X2, y2, test_size=0.2, stratify=y2, random_state=21, shuffle=True)
